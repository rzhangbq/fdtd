...
Suppose the source is added only once in the second substep and evaluated at the midpoint, with \(S_{E,1}^n=0\), \(S_{E,2}^n=\Delta t S^{n+\frac{1}{2}}\), \(S_{E,1}^{n-1}=0\), and \(S_{E,2}^{n-1}=\Delta t S^{n-\frac{1}{2}}\). Then the source terms in equation~\eqref{eq:discrete-wave-equation} reduce to
\begin{equation}
    \Delta t\left[\left(1-\gamma\delta_x^{E,2}\right)S^{n+\frac{1}{2}}-\left(1+\gamma\delta_x^{E,2}\right)S^{n-\frac{1}{2}}\right]. \label{eq:single-midpoint-source-reduction}
\end{equation}
Therefore equation~\eqref{eq:discrete-wave-equation} becomes
\begin{equation}
    \frac{E_z^{n+1}-2E_z^n+E_z^{n-1}}{\Delta t^2}=c^2\delta_x^{E,2}\left(\frac{E_z^{n+1}+2E_z^n+E_z^{n-1}}{4}\right)+\frac{\left(1-\gamma\delta_x^{E,2}\right)S^{n+\frac{1}{2}}-\left(1+\gamma\delta_x^{E,2}\right)S^{n-\frac{1}{2}}}{\Delta t}. \label{eq:wave-equation-single-midpoint-source}
\end{equation}
Assume \(S^{n+a}=S(t_n+a\Delta t)\) and \(F^n=S_t(t_n)\). The source approximation in equation~\eqref{eq:wave-equation-single-midpoint-source} has the Taylor expansion
\begin{equation}
    F^n-\frac{2\gamma}{\Delta t}\delta_x^{E,2}S^n+\frac{\Delta t^2}{24}S_{ttt}^n+O(\Delta t^3), \qquad \mathrm{error}=-\frac{2\gamma}{\Delta t}\delta_x^{E,2}S^n+\frac{\Delta t^2}{24}S_{ttt}^n+O(\Delta t^3). \label{eq:single-midpoint-source-error}
\end{equation}
Since \(\gamma=O(\Delta t^2)\), the single midpoint-source case is first-order accurate for a spatially varying \(S\), but remains second-order when \(\delta_x^{E,2}S=0\).
...
Assume \(S(x,t)=\widehat{S}e^{i(kx+\omega t)}\), and define \(\theta=\omega\Delta t\), \(\delta_x^{E,2}e^{ikx}=\lambda_xe^{ikx}\), and \(q=-\gamma\lambda_x\geq0\). Substituting into the source term on the right-hand side of equation~\eqref{eq:wave-equation-single-midpoint-source} gives
\begin{equation}
    \begin{aligned} F_{\mathrm{mid}}^n & =\frac{\left(1-\gamma\delta_x^{E,2}\right)S^{n+\frac{1}{2}}-\left(1+\gamma\delta_x^{E,2}\right)S^{n-\frac{1}{2}}}{\Delta t} \\
                                   & =\frac{\widehat{S}e^{i(kx+\omega t_n)}}{\Delta t}\left[\left(1+q\right)e^{i\theta/2}-\left(1-q\right)e^{-i\theta/2}\right] \\
...
\end{equation}
so \(\widehat{F}_{\mathrm{mid}}=\frac{2\widehat{S}}{\Delta t}\left[q\cos\left(\frac{\theta}{2}\right)+i\sin\left(\frac{\theta}{2}\right)\right]\).
...
\begin{equation}
    \frac{\left|\widehat{F}_{\mathrm{quarter}}\right|}{\left|\widehat{F}_{\mathrm{mid}}\right|}=\frac{\left|\left(1-q\right)\sin\left(\frac{\theta}{4}\right)+\left(1+q\right)\sin\left(\frac{3\theta}{4}\right)\right|}{2\sqrt{q^2\cos^2\left(\frac{\theta}{2}\right)+\sin^2\left(\frac{\theta}{2}\right)}}. \label{eq:source-magnitude-ratio}
\end{equation}
