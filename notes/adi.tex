...
Here we derive the convolutional PML boundary conditions for the ADI scheme from \cite{gedney2001perfectly}. The PML cells belong to the same Yee grid as the physical cells. The electric field is advanced by the tridiagonal systems, and the magnetic flux density is advanced after each electric solve; the magnetic field in the equations below is obtained from $H=B/\mu$.
...
The magnetic auxiliary variables are stored in the unscaled recursive-convolution form; therefore $D_b$ is applied when they enter the magnetic update, at the same staggered location as the corresponding magnetic field.
