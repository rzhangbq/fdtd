...
\section{Perfectly Matched Layer Boundary Conditions}
Here we derive the convolutional PML boundary conditions for the ADI scheme from \cite{gedney2001perfectly}. The PML cells belong to the same contiguous Yee grid as the physical cells; this implementation does not construct the split-field \texttt{pml[lev]} boxes used by the FDTD solver. With \texttt{warpx.do\_pml\_in\_domain=1}, the outer $N_{\mathrm{pml}}$ cells of the user-supplied domain form the PML. With \texttt{warpx.do\_pml\_in\_domain=0}, the default, the geometry is first extended by $N_{\mathrm{pml}}$ cells on each face marked as PML, so that the absorbing layer lies outside the user-supplied physical domain while remaining part of the same ADI grid. The current implementation is restricted to three dimensions and one AMR level. A direction containing a PML must be nonperiodic and must have a PML or PEC boundary at both outer walls. These walls provide a PEC-like termination: tangential $E$ and normal $B$ are set to zero there.

The electric field is advanced by the tridiagonal systems, and the magnetic flux density is advanced after each electric solve; the magnetic field in the equations below is obtained from $H=B/\mu$.

For a derivative in direction $s$, let $D_s H$ denote the backward Yee difference at an electric-field location and $D_s E$ the forward Yee difference at a magnetic-field location. The final subscript $s$ of $\psi_{e\alpha s}$ and $\psi_{h\beta s}$ identifies the derivative direction, not the component being differentiated. For example, $\psi_{exy}$ corresponds to $D_y H_z$ in the $E_x$ equation, while $\psi_{hzy}$ corresponds to $D_y E_x$ in the $H_z$ equation. The corresponding component pairs are
\begin{align}
    (\psi_{exy},\psi_{exz},\psi_{eyz},\psi_{eyx},\psi_{ezx},\psi_{ezy})
    &\longleftrightarrow (D_yH_z,D_zH_y,D_zH_x,D_xH_z,D_xH_y,D_yH_x), \\
    (\psi_{hzy},\psi_{hzx},\psi_{hxz},\psi_{hxy},\psi_{hyx},\psi_{hyz})
    &\longleftrightarrow (D_yE_x,D_xE_y,D_zE_y,D_yE_z,D_xE_z,D_zE_x).
\end{align}

At each Yee location the recursive-convolution variable satisfies
\begin{equation}
    \psi_s^+ = b_s\psi_s^- + a_s D_sF,
    \qquad
    S_s=\frac{1}{\kappa_s}+a_s.
    \label{eq:adi-pml-implicit-stretch}
\end{equation}
Here $-$ and $+$ denote the beginning and end of one ADI half-step. If the differentiated field $F$ is known, the auxiliary variable is updated first and the stretched derivative is $\kappa_s^{-1}D_sF+\psi_s^+$. If $F$ is the unknown field, then
\begin{equation}
    \frac{1}{\kappa_s}D_sF^+ + \psi_s^+
    = S_sD_sF^+ + b_s\psi_s^-.
    \label{eq:adi-pml-eliminated-stretch}
\end{equation}
The $a_sD_sF^+$ term is therefore included in the implicit tridiagonal coefficients, rather than treated as an old auxiliary-variable source.
...
\end{align}
while outside the PML and at the discrete inner interface $\rho_s=0$,
\begin{equation}
    \sigma_s=0, \qquad \alpha_s=0, \qquad \kappa_s=1, \qquad a_s=0, \qquad b_s=1, \qquad S_s=1.
\end{equation}
Unless the maximum conductivity is prescribed directly, it may be chosen from a target reflection coefficient $R$ as
\begin{equation}
    \sigma_{s,\max}
    =
...
\end{equation}
Here $R$ sets the conductivity profile; it is not necessarily the achieved numerical reflection coefficient.

The coefficients $\kappa_s$, $a_s$, $b_s$, and $S_s$ in these equations are evaluated at the Yee location of the derivative: $x_s=x_{s,\mathrm{lo}}+i_s\Delta s$ on a nodal layout and $x_s=x_{s,\mathrm{lo}}+(i_s+\tfrac12)\Delta s$ on a cell-centered layout. In particular, the electric and magnetic factors $S_{s,E}$ and $S_{s,H}$ below are evaluated at their distinct staggered locations. Outside the PML, $b_s=1$ and $S_s=1$.

\subsection{First ADI Half-Step}

Using the same formulation as the ADI tridiagonal systems, the three first-half-step equations have the following common form. Set $(E_\alpha,H_\beta,H_\gamma,E_\delta;t,u,v)$ equal to
\begin{equation}
    (E_x,H_z,H_y,E_y;y,z,x),\qquad
    (E_y,H_x,H_z,E_z;z,x,y),\qquad
    (E_z,H_y,H_x,E_x;x,y,z),
    \label{eq:first-pml-component-pairs}
\end{equation}
respectively. The direction $t$ is implicit, $u$ is the explicit direction in the electric curl, and $v$ is the explicit direction in the magnetic curl. A superscript $-$ means $n$ and a superscript $+$ means $n+\tfrac12$ in this subsection. Define the known explicit-direction terms
\begin{align}
    Q_{e,\alpha u}^+ &= \frac{1}{\kappa_{u,E_\alpha}}D_uH_\gamma^-+\psi_{e\alpha u}^+, \\
    Q_{h,\beta v}^+ &= \frac{1}{\kappa_{v,H_\beta}}D_vE_\delta^-+\psi_{h\beta v}^+,
\end{align}
and the right-hand side
\begin{align}
    \mathcal R_{\alpha 1}
    &=p_{\alpha 1}E_\alpha^-
      +S_{t,E_\alpha}D_tH_\beta^-
      -Q_{e,\alpha u}^+
      +b_{t,E_\alpha}\psi_{e\alpha t}^- \notag\\
    &\quad
      +S_{t,E_\alpha}D_t\!\left\{
        D_{b,H_\beta}\left[
          b_{t,H_\beta}\psi_{h\beta t}^-
          -Q_{h,\beta v}^+
        \right]\right\}.
    \label{eq:first-pml-rhs}
\end{align}
The explicit auxiliaries $\psi_{e\alpha u}^+$ and $\psi_{h\beta v}^+$ are updated from $H^n$ and $E^n$, respectively, before these right-hand sides are formed. The implicit auxiliary $\psi_{h\beta t}^+$ depends on $E_\alpha^{n+\frac12}$; its new-field part is included in $S_{t,H_\beta}$.
...
\begin{equation}
    ...
    =\mathcal R_{x1,i+\tfrac12,j,k}.
    \label{eq:first-e-ex-pml-adi-tridiagonal}
\end{equation}
Here
\begin{align}
    \alpha_{x1}&=\frac{D_{b,i+\tfrac12,j-\tfrac12,k}
      S_{y,E_x,i+\tfrac12,j,k}S_{y,H_z,i+\tfrac12,j-\tfrac12,k}}{\Delta y^2},\\
    \beta_{x1}&=\frac{1}{C_{b,i+\tfrac12,j,k}}+\alpha_{x1}+\gamma_{x1},\\
    \gamma_{x1}&=\frac{D_{b,i+\tfrac12,j+\tfrac12,k}
      S_{y,E_x,i+\tfrac12,j,k}S_{y,H_z,i+\tfrac12,j+\tfrac12,k}}{\Delta y^2},\\
    p_{x1}&=\frac{C_{a,i+\tfrac12,j,k}}{C_{b,i+\tfrac12,j,k}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    H_z^{n+\frac12}=H_z^n+D_{b,H_z}
    \left(\frac{1}{\kappa_{y,H_z}}D_yE_x^{n+\frac12}
          +\psi_{hzy}^{n+\frac12}-Q_{h,zx}^{n+\frac12}\right).
...
\begin{equation}
    -\alpha_{y1}E_{y,i,j+\tfrac12,k-1}^{n+\frac12}
    +\beta_{y1}E_{y,i,j+\tfrac12,k}^{n+\frac12}
    -\gamma_{y1}E_{y,i,j+\tfrac12,k+1}^{n+\frac12}
    =\mathcal R_{y1,i,j+\tfrac12,k}.
    \label{eq:first-e-ey-pml-adi-tridiagonal}
\end{equation}
Here
\begin{align}
    \alpha_{y1}&=\frac{D_{b,i,j+\tfrac12,k-\tfrac12}
      S_{z,E_y,i,j+\tfrac12,k}S_{z,H_x,i,j+\tfrac12,k-\tfrac12}}{\Delta z^2},\\
    \beta_{y1}&=\frac{1}{C_{b,i,j+\tfrac12,k}}+\alpha_{y1}+\gamma_{y1},\\
    \gamma_{y1}&=\frac{D_{b,i,j+\tfrac12,k+\tfrac12}
      S_{z,E_y,i,j+\tfrac12,k}S_{z,H_x,i,j+\tfrac12,k+\tfrac12}}{\Delta z^2},\\
    p_{y1}&=\frac{C_{a,i,j+\tfrac12,k}}{C_{b,i,j+\tfrac12,k}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    H_x^{n+\frac12}=H_x^n+D_{b,H_x}
    \left(\frac{1}{\kappa_{z,H_x}}D_zE_y^{n+\frac12}
          +\psi_{hxz}^{n+\frac12}-Q_{h,xy}^{n+\frac12}\right).
...
\begin{equation}
    -\alpha_{z1}E_{z,i-1,j,k+\tfrac12}^{n+\frac12}
    +\beta_{z1}E_{z,i,j,k+\tfrac12}^{n+\frac12}
    -\gamma_{z1}E_{z,i+1,j,k+\tfrac12}^{n+\frac12}
    =\mathcal R_{z1,i,j,k+\tfrac12}.
    \label{eq:first-e-ez-pml-adi-tridiagonal}
\end{equation}
Here
\begin{align}
    \alpha_{z1}&=\frac{D_{b,i-\tfrac12,j,k+\tfrac12}
      S_{x,E_z,i,j,k+\tfrac12}S_{x,H_y,i-\tfrac12,j,k+\tfrac12}}{\Delta x^2},\\
    \beta_{z1}&=\frac{1}{C_{b,i,j,k+\tfrac12}}+\alpha_{z1}+\gamma_{z1},\\
    \gamma_{z1}&=\frac{D_{b,i+\tfrac12,j,k+\tfrac12}
      S_{x,E_z,i,j,k+\tfrac12}S_{x,H_y,i+\tfrac12,j,k+\tfrac12}}{\Delta x^2},\\
    p_{z1}&=\frac{C_{a,i,j,k+\tfrac12}}{C_{b,i,j,k+\tfrac12}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    H_y^{n+\frac12}=H_y^n+D_{b,H_y}
    \left(\frac{1}{\kappa_{x,H_y}}D_xE_z^{n+\frac12}
          +\psi_{hyx}^{n+\frac12}-Q_{h,yz}^{n+\frac12}\right).
    \label{eq:adi-first-hy-pml}
\end{equation}

When the PML is disabled, or equivalently when $\kappa_s=1$, $a_s=0$, $b_s=1$, and all PML auxiliary variables vanish, equations \eqref{eq:first-e-ex-pml-adi-tridiagonal}, \eqref{eq:first-e-ey-pml-adi-tridiagonal}, and \eqref{eq:first-e-ez-pml-adi-tridiagonal} reduce to \eqref{eq:first-e-ex-adi-tridiagonal}, \eqref{eq:first-e-ey-adi-tridiagonal}, and \eqref{eq:first-e-ez-adi-tridiagonal}, respectively. Likewise, \eqref{eq:adi-first-hz-pml}, \eqref{eq:adi-first-hx-pml}, and \eqref{eq:adi-first-hy-pml} reduce to \eqref{eq:adi-first-hz}, \eqref{eq:adi-first-hx}, and \eqref{eq:adi-first-hy}.

\subsection{First PML Auxiliary Variable Half-Step}

The PML auxiliary variables are advanced over the first half-step from time level $n$ to $n+\tfrac12$. The explicit electric auxiliary variables $\psi_{exz}$, $\psi_{eyx}$, and $\psi_{ezy}$ are updated from the magnetic fields at time level $n$ before the electric solve. The explicit magnetic auxiliary variables $\psi_{hzx}$, $\psi_{hxy}$, and $\psi_{hyz}$ are updated from the electric fields at time level $n$ at the same point. After the electric solve, $\psi_{hzy}$, $\psi_{hxz}$, and $\psi_{hyx}$ are updated from $E^{n+\frac12}$ and used in the magnetic update. The magnetic field is then obtained from $B^{n+\frac12}/\mu$, and $\psi_{exy}$, $\psi_{eyz}$, and $\psi_{ezx}$ are updated from that field. Thus the known and unknown derivative contributions use the time levels in \eqref{eq:adi-pml-implicit-stretch} and \eqref{eq:adi-pml-eliminated-stretch}, respectively.

\subsection{Second ADI Half-Step}

Using the same formulation as the ADI tridiagonal systems, the three second-half-step equations have the same form as \eqref{eq:first-pml-rhs}, with the implicit and explicit directions exchanged. Set $(E_\alpha,H_\beta,H_\gamma,E_\delta;t,u,v)$ equal to
\begin{equation}
    (E_x,H_y,H_z,E_z;z,y,x),\qquad
    (E_y,H_z,H_x,E_x;x,z,y),\qquad
    (E_z,H_x,H_y,E_y;y,x,z),
    \label{eq:second-pml-component-pairs}
\end{equation}
respectively. In this subsection a superscript $-$ means $n+\tfrac12$ and a superscript $+$ means $n+1$. The magnetic and electric curls have the opposite orientation, so $\eta=-1$ and the right-hand side is
\begin{align}
    \mathcal R_{\alpha 2}
    &=p_{\alpha 2}E_\alpha^-
      -S_{t,E_\alpha}D_tH_\beta^-
    ...
    &\quad
      +S_{t,E_\alpha}D_t\!\left\{
        D_{b,H_\beta}\left[
          ...
          -Q_{h,\beta v}^+
        \right]\right\},
    \label{eq:second-pml-rhs}
\end{align}
where $Q_{e,\alpha u}^+$ and $Q_{h,\beta v}^+$ are defined as above using the fields at time level $n+\tfrac12$.
...
\begin{equation}
    ...
    =\mathcal R_{x2,i+\tfrac12,j,k}.
    \label{eq:second-e-ex-pml-adi-tridiagonal}
\end{equation}
Here
\begin{align}
    \alpha_{x2}&=\frac{D_{b,i+\tfrac12,j,k-\tfrac12}
      S_{z,E_x,i+\tfrac12,j,k}S_{z,H_y,i+\tfrac12,j,k-\tfrac12}}{\Delta z^2},\\
    \beta_{x2}&=\frac{1}{C_{b,i+\tfrac12,j,k}}+\alpha_{x2}+\gamma_{x2},\\
    \gamma_{x2}&=\frac{D_{b,i+\tfrac12,j,k+\tfrac12}
      S_{z,E_x,i+\tfrac12,j,k}S_{z,H_y,i+\tfrac12,j,k+\tfrac12}}{\Delta z^2},\\
    p_{x2}&=\frac{C_{a,i+\tfrac12,j,k}}{C_{b,i+\tfrac12,j,k}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    H_y^{n+1}=H_y^{n+\frac12}-D_{b,H_y}
    \left(\frac{1}{\kappa_{z,H_y}}D_zE_x^{n+1}
          +\psi_{hyz}^{n+1}-Q_{h,yx}^{n+1}\right).
...
\begin{equation}
    -\alpha_{y2}E_{y,i-1,j+\tfrac12,k}^{n+1}
    +\beta_{y2}E_{y,i,j+\tfrac12,k}^{n+1}
    -\gamma_{y2}E_{y,i+1,j+\tfrac12,k}^{n+1}
    =\mathcal R_{y2,i,j+\tfrac12,k}.
    \label{eq:second-e-ey-pml-adi-tridiagonal}
\end{equation}
Here
\begin{align}
    \alpha_{y2}&=\frac{D_{b,i-\tfrac12,j+\tfrac12,k}
      S_{x,E_y,i,j+\tfrac12,k}S_{x,H_z,i-\tfrac12,j+\tfrac12,k}}{\Delta x^2},\\
    \beta_{y2}&=\frac{1}{C_{b,i,j+\tfrac12,k}}+\alpha_{y2}+\gamma_{y2},\\
    \gamma_{y2}&=\frac{D_{b,i+\tfrac12,j+\tfrac12,k}
      S_{x,E_y,i,j+\tfrac12,k}S_{x,H_z,i+\tfrac12,j+\tfrac12,k}}{\Delta x^2},\\
    p_{y2}&=\frac{C_{a,i,j+\tfrac12,k}}{C_{b,i,j+\tfrac12,k}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    H_z^{n+1}=H_z^{n+\frac12}-D_{b,H_z}
    \left(\frac{1}{\kappa_{x,H_z}}D_xE_y^{n+1}
          +\psi_{hzx}^{n+1}-Q_{h,zy}^{n+1}\right).
...
\begin{equation}
    -\alpha_{z2}E_{z,i,j-1,k+\tfrac12}^{n+1}
    +\beta_{z2}E_{z,i,j,k+\tfrac12}^{n+1}
    -\gamma_{z2}E_{z,i,j+1,k+\tfrac12}^{n+1}
    =\mathcal R_{z2,i,j,k+\tfrac12}.
    \label{eq:second-e-ez-pml-adi-tridiagonal}
\end{equation}
Here
\begin{align}
    \alpha_{z2}&=\frac{D_{b,i,j-\tfrac12,k+\tfrac12}
      S_{y,E_z,i,j,k+\tfrac12}S_{y,H_x,i,j-\tfrac12,k+\tfrac12}}{\Delta y^2},\\
    \beta_{z2}&=\frac{1}{C_{b,i,j,k+\tfrac12}}+\alpha_{z2}+\gamma_{z2},\\
    \gamma_{z2}&=\frac{D_{b,i,j+\tfrac12,k+\tfrac12}
      S_{y,E_z,i,j,k+\tfrac12}S_{y,H_x,i,j+\tfrac12,k+\tfrac12}}{\Delta y^2},\\
    p_{z2}&=\frac{C_{a,i,j,k+\tfrac12}}{C_{b,i,j,k+\tfrac12}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    H_x^{n+1}=H_x^{n+\frac12}-D_{b,H_x}
    \left(\frac{1}{\kappa_{y,H_x}}D_yE_z^{n+1}
          +\psi_{hxy}^{n+1}-Q_{h,xz}^{n+1}\right).
...
The PML auxiliary variables are advanced over the second half-step from time level $n+\tfrac12$ to $n+1$. The notation for $\psi_{e\alpha s}$ and $\psi_{h\beta s}$ is the same as in the first PML auxiliary-variable half-step. The explicit electric auxiliary variables $\psi_{exy}$, $\psi_{eyz}$, and $\psi_{ezx}$ are updated from $H^{n+\frac12}$ before the electric solve. The explicit magnetic auxiliary variables $\psi_{hzy}$, $\psi_{hxz}$, and $\psi_{hyx}$ are updated from $E^{n+\frac12}$ at the same point. After the electric solve, $\psi_{hyz}$, $\psi_{hzx}$, and $\psi_{hxy}$ are updated from $E^{n+1}$ and used in the magnetic update. The magnetic field is then obtained from $B^{n+1}/\mu$, and $\psi_{exz}$, $\psi_{eyx}$, and $\psi_{ezy}$ are updated from that field.

The magnetic auxiliary variables are stored in the unscaled recursive-convolution form. Consequently, $D_b$ is applied at the same staggered location as the corresponding magnetic field, both in the direct magnetic update and in the magnetic terms eliminated into the electric right-hand side.
