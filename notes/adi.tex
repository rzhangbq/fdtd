...
The field update uses the auxiliary variable from the beginning of the half-step. The stretched derivative is
\begin{equation}
    \frac{1}{\kappa_s}D_sF+\psi_s^-,
    \label{eq:adi-pml-lagged-stretch}
\end{equation}
where $-$ denotes the start of the half-step. After $E$ and $B$ have both been advanced, every auxiliary is updated by
\begin{equation}
    \psi_s^+ = b_s\psi_s^- + a_s D_sF.
    \label{eq:adi-pml-auxiliary-update}
\end{equation}
An explicit-direction auxiliary uses the field from the start of the half-step, and an implicit-direction auxiliary uses the field just obtained. The coefficients $a_s$ and $b_s$ therefore appear only in this update. The tridiagonal matrix contains $1/\kappa_s$, not $a_s$.
...
\begin{equation}
    \sigma_s=0, \qquad \alpha_s=0, \qquad \kappa_s=1, \qquad
    a_s=0, \qquad b_s=1.
\end{equation}
...
The coefficients $\kappa_s$, $a_s$, and $b_s$ in these equations are evaluated at the Yee location of the derivative: $x_s=x_{s,\mathrm{lo}}+i_s\Delta s$ on a nodal layout and $x_s=x_{s,\mathrm{lo}}+(i_s+\tfrac12)\Delta s$ on a cell-centered layout. In particular, the electric and magnetic factors $\kappa_{s,E}$ and $\kappa_{s,H}$ below are evaluated at their distinct staggered locations. Outside the PML, $\kappa_s=1$, $a_s=0$, and $b_s=1$.
...
\begin{align}
    \mathcal R_{\alpha 1}
    &=p_{\alpha 1}E_\alpha^-
      +\frac{1}{\kappa_{t,E_\alpha}}D_tH_\beta^-
      -Q_{e,\alpha u}^-
      +\psi_{e\alpha t}^- \notag\\
    &\quad
      +\frac{1}{\kappa_{t,E_\alpha}}D_t\!\left\{
        D_{b,H_\beta}\left[
          \psi_{h\beta t}^-
          -Q_{h,\beta v}^-
        \right]\right\}.
...
\end{align}
Every auxiliary in this right-hand side is the value at the start of the half-step. The auxiliaries are not updated until after $E$ and $B$.
...
Here
\begin{align}
    \alpha_{x1}&=\frac{D_{b,i+\tfrac12,j-\tfrac12,k}}{\kappa_{y,E_x,i+\tfrac12,j,k}\kappa_{y,H_z,i+\tfrac12,j-\tfrac12,k}\Delta y^2},\\
    \beta_{x1}&=\frac{1}{C_{b,i+\tfrac12,j,k}}+\alpha_{x1}+\gamma_{x1},\\
    \gamma_{x1}&=\frac{D_{b,i+\tfrac12,j+\tfrac12,k}}{\kappa_{y,E_x,i+\tfrac12,j,k}\kappa_{y,H_z,i+\tfrac12,j+\tfrac12,k}\Delta y^2},\\
    p_{x1}&=\frac{C_{a,i+\tfrac12,j,k}}{C_{b,i+\tfrac12,j,k}}.
\end{align}
...
Here
\begin{align}
    \alpha_{y1}&=\frac{D_{b,i,j+\tfrac12,k-\tfrac12}}{\kappa_{z,E_y,i,j+\tfrac12,k}\kappa_{z,H_x,i,j+\tfrac12,k-\tfrac12}\Delta z^2},\\
    \beta_{y1}&=\frac{1}{C_{b,i,j+\tfrac12,k}}+\alpha_{y1}+\gamma_{y1},\\
    \gamma_{y1}&=\frac{D_{b,i,j+\tfrac12,k+\tfrac12}}{\kappa_{z,E_y,i,j+\tfrac12,k}\kappa_{z,H_x,i,j+\tfrac12,k+\tfrac12}\Delta z^2},\\
    p_{y1}&=\frac{C_{a,i,j+\tfrac12,k}}{C_{b,i,j+\tfrac12,k}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
...
Here
\begin{align}
    \alpha_{z1}&=\frac{D_{b,i-\tfrac12,j,k+\tfrac12}}{\kappa_{x,E_z,i,j,k+\tfrac12}\kappa_{x,H_y,i-\tfrac12,j,k+\tfrac12}\Delta x^2},\\
    \beta_{z1}&=\frac{1}{C_{b,i,j,k+\tfrac12}}+\alpha_{z1}+\gamma_{z1},\\
    \gamma_{z1}&=\frac{D_{b,i+\tfrac12,j,k+\tfrac12}}{\kappa_{x,E_z,i,j,k+\tfrac12}\kappa_{x,H_y,i+\tfrac12,j,k+\tfrac12}\Delta x^2},\\
    p_{z1}&=\frac{C_{a,i,j,k+\tfrac12}}{C_{b,i,j,k+\tfrac12}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    ...
    \left(\frac{1}{\kappa_{x,H_y}}D_xE_z^{n+\frac12}
          +\psi_{hyx}^{n}-Q_{h,yz}^{n}\right).
    \label{eq:adi-first-hy-pml}
\end{equation}

When the PML is disabled, or equivalently when $\kappa_s=1$ and all PML auxiliary variables vanish, equations \eqref{eq:first-e-ex-pml-adi-tridiagonal}, \eqref{eq:first-e-ey-pml-adi-tridiagonal}, and \eqref{eq:first-e-ez-pml-adi-tridiagonal} reduce to \eqref{eq:first-e-ex-adi-tridiagonal}, \eqref{eq:first-e-ey-adi-tridiagonal}, and \eqref{eq:first-e-ez-adi-tridiagonal}, respectively. Likewise, \eqref{eq:adi-first-hz-pml}, \eqref{eq:adi-first-hx-pml}, and \eqref{eq:adi-first-hy-pml} reduce to \eqref{eq:adi-first-hz}, \eqref{eq:adi-first-hx}, and \eqref{eq:adi-first-hy}.

\subsection{First PML Auxiliary Variable Half-Step}

The PML auxiliary variables advance from time level $n$ to $n+\tfrac12$ only after $E^{n+\frac12}$ and $B^{n+\frac12}$ are known. The explicit magnetic auxiliaries $\psi_{hzx}$, $\psi_{hxy}$, and $\psi_{hyz}$ are updated from $E^n$. The implicit magnetic auxiliaries $\psi_{hzy}$, $\psi_{hxz}$, and $\psi_{hyx}$ are updated from $E^{n+\frac12}$. The explicit electric auxiliaries $\psi_{exz}$, $\psi_{eyx}$, and $\psi_{ezy}$ are updated from $H^n$, and the implicit electric auxiliaries $\psi_{exy}$, $\psi_{eyz}$, and $\psi_{ezx}$ are updated from $H^{n+\frac12}=B^{n+\frac12}/\mu$. These updates use \eqref{eq:adi-pml-auxiliary-update}.
...
\end{equation}
respectively. In this subsection a superscript $-$ means $n+\tfrac12$ and a superscript $+$ means $n+1$. The magnetic and electric curls have the opposite orientation, and the right-hand side is
\begin{align}
    \mathcal R_{\alpha 2}
    &=p_{\alpha 2}E_\alpha^-
      -\frac{1}{\kappa_{t,E_\alpha}}D_tH_\beta^-
...
\end{align}
where $Q_{e,\alpha u}^-$ and $Q_{h,\beta v}^-$ are defined as above using the fields and auxiliaries at time level $n+\tfrac12$.
...
Here
\begin{align}
    \alpha_{x2}&=\frac{D_{b,i+\tfrac12,j,k-\tfrac12}}{\kappa_{z,E_x,i+\tfrac12,j,k}\kappa_{z,H_y,i+\tfrac12,j,k-\tfrac12}\Delta z^2},\\
    \beta_{x2}&=\frac{1}{C_{b,i+\tfrac12,j,k}}+\alpha_{x2}+\gamma_{x2},\\
    \gamma_{x2}&=\frac{D_{b,i+\tfrac12,j,k+\tfrac12}}{\kappa_{z,E_x,i+\tfrac12,j,k}\kappa_{z,H_y,i+\tfrac12,j,k+\tfrac12}\Delta z^2},\\
    p_{x2}&=\frac{C_{a,i+\tfrac12,j,k}}{C_{b,i+\tfrac12,j,k}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    ...
    \left(\frac{1}{\kappa_{z,H_y}}D_zE_x^{n+1}
          +\psi_{hyz}^{n+\frac12}-Q_{h,yx}^{n+\frac12}\right).
...
Here
\begin{align}
    \alpha_{y2}&=\frac{D_{b,i-\tfrac12,j+\tfrac12,k}}{\kappa_{x,E_y,i,j+\tfrac12,k}\kappa_{x,H_z,i-\tfrac12,j+\tfrac12,k}\Delta x^2},\\
    \beta_{y2}&=\frac{1}{C_{b,i,j+\tfrac12,k}}+\alpha_{y2}+\gamma_{y2},\\
    \gamma_{y2}&=\frac{D_{b,i+\tfrac12,j+\tfrac12,k}}{\kappa_{x,E_y,i,j+\tfrac12,k}\kappa_{x,H_z,i+\tfrac12,j+\tfrac12,k}\Delta x^2},\\
    p_{y2}&=\frac{C_{a,i,j+\tfrac12,k}}{C_{b,i,j+\tfrac12,k}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    ...
    \left(\frac{1}{\kappa_{x,H_z}}D_xE_y^{n+1}
          +\psi_{hzx}^{n+\frac12}-Q_{h,zy}^{n+\frac12}\right).
...
Here
\begin{align}
    \alpha_{z2}&=\frac{D_{b,i,j-\tfrac12,k+\tfrac12}}{\kappa_{y,E_z,i,j,k+\tfrac12}\kappa_{y,H_x,i,j-\tfrac12,k+\tfrac12}\Delta y^2},\\
    \beta_{z2}&=\frac{1}{C_{b,i,j,k+\tfrac12}}+\alpha_{z2}+\gamma_{z2},\\
    \gamma_{z2}&=\frac{D_{b,i,j+\tfrac12,k+\tfrac12}}{\kappa_{y,E_z,i,j,k+\tfrac12}\kappa_{y,H_x,i,j+\tfrac12,k+\tfrac12}\Delta y^2},\\
    p_{z2}&=\frac{C_{a,i,j,k+\tfrac12}}{C_{b,i,j,k+\tfrac12}}.
\end{align}
and the PML magnetic update can be written as
\begin{equation}
    ...
    \left(\frac{1}{\kappa_{y,H_x}}D_yE_z^{n+1}
          +\psi_{hxy}^{n+\frac12}-Q_{h,xz}^{n+\frac12}\right).
...
The PML auxiliary variables advance from time level $n+\tfrac12$ to $n+1$ only after $E^{n+1}$ and $B^{n+1}$ are known. The explicit magnetic auxiliaries $\psi_{hzy}$, $\psi_{hxz}$, and $\psi_{hyx}$ are updated from $E^{n+\frac12}$. The implicit magnetic auxiliaries $\psi_{hyz}$, $\psi_{hzx}$, and $\psi_{hxy}$ are updated from $E^{n+1}$. The explicit electric auxiliaries $\psi_{exy}$, $\psi_{eyz}$, and $\psi_{ezx}$ are updated from $H^{n+\frac12}$, and the implicit electric auxiliaries $\psi_{exz}$, $\psi_{eyx}$, and $\psi_{ezy}$ are updated from $H^{n+1}=B^{n+1}/\mu$.
