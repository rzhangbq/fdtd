...
\begin{align}
    \begin{split}
        -\alpha_{x1}E_{x,i+\tfrac12,j-1,k}^{n+\frac12} + \beta_{x1}E_{x,i+\tfrac12,j,k}^{n+\frac12} - \gamma_{x1}E_{x,i+\tfrac12,j+1,k}^{n+\frac12}
         ...
         & \quad -\frac{r_{x1}}{\kappa_{y,i+\tfrac12,j,k}\Delta y}\Big(\psi^{n}_{h\,zy,i+\tfrac12,j+\tfrac12,k} - \psi^{n}_{h\,zx,i+\tfrac12,j+\tfrac12,k}\Big)           \\
         & \quad +\frac{q_{x1}}{\kappa_{y,i+\tfrac12,j,k}\Delta y}\Big(\psi^{n}_{h\,zy,i+\tfrac12,j-\tfrac12,k} - \psi^{n}_{h\,zx,i+\tfrac12,j-\tfrac12,k}\Big)           \\
         & \quad +\Big(\psi^{n}_{e\,xz,i+\tfrac12,j,k}-\psi^{n}_{e\,xy,i+\tfrac12,j,k}\Big).
    \end{split}
...
\begin{equation}
    \begin{aligned}
        ...
        \Big(E_{y,i+1,j+\tfrac12,k}^{n}-E_{y,i,j+\tfrac12,k}^{n}\Big)                 \\
         & \quad
        +r_{x1}\psi^{n}_{h\,zy,i+\tfrac12,j+\tfrac12,k}
        -r_{x1}\psi^{n}_{h\,zx,i+\tfrac12,j+\tfrac12,k}.
    \end{aligned}
...
\begin{align}
    \begin{split}
        -\alpha_{y1}E_{y,i,j+\tfrac12,k-1}^{n+\frac12} + \beta_{y1}E_{y,i,j+\tfrac12,k}^{n+\frac12} - \gamma_{y1}E_{y,i,j+\tfrac12,k+1}^{n+\frac12}
         ...
         & \quad -\frac{r_{y1}}{\kappa_{z,i,j+\tfrac12,k}\Delta z}\Big(\psi^{n}_{h\,xz,i,j+\tfrac12,k+\tfrac12} - \psi^{n}_{h\,xy,i,j+\tfrac12,k+\tfrac12}\Big)           \\
         & \quad +\frac{q_{y1}}{\kappa_{z,i,j+\tfrac12,k}\Delta z}\Big(\psi^{n}_{h\,xz,i,j+\tfrac12,k-\tfrac12} - \psi^{n}_{h\,xy,i,j+\tfrac12,k-\tfrac12}\Big)           \\
         & \quad +\Big(\psi^{n}_{e\,yx,i,j+\tfrac12,k}-\psi^{n}_{e\,yz,i,j+\tfrac12,k}\Big).
    \end{split}
...
\begin{equation}
    \begin{aligned}
        ...
        \Big(E_{z,i,j+1,k+\tfrac12}^{n}-E_{z,i,j,k+\tfrac12}^{n}\Big)                 \\
         & \quad
        +r_{y1}\psi^{n}_{h\,xz,i,j+\tfrac12,k+\tfrac12}
        -r_{y1}\psi^{n}_{h\,xy,i,j+\tfrac12,k+\tfrac12}.
    \end{aligned}
...
\begin{align}
    \begin{split}
        -\alpha_{z1}E_{z,i-1,j,k+\tfrac12}^{n+\frac12} + \beta_{z1}E_{z,i,j,k+\tfrac12}^{n+\frac12} - \gamma_{z1}E_{z,i+1,j,k+\tfrac12}^{n+\frac12}
         ...
         & \quad -\frac{r_{z1}}{\kappa_{x,i,j,k+\tfrac12}\Delta x}\Big(\psi^{n}_{h\,yx,i+\tfrac12,j,k+\tfrac12} - \psi^{n}_{h\,yz,i+\tfrac12,j,k+\tfrac12}\Big)           \\
         & \quad +\frac{q_{z1}}{\kappa_{x,i,j,k+\tfrac12}\Delta x}\Big(\psi^{n}_{h\,yx,i-\tfrac12,j,k+\tfrac12} - \psi^{n}_{h\,yz,i-\tfrac12,j,k+\tfrac12}\Big)           \\
         & \quad +\Big(\psi^{n}_{e\,zy,i,j,k+\tfrac12}-\psi^{n}_{e\,zx,i,j,k+\tfrac12}\Big).
    \end{split}
...
\begin{equation}
    \begin{aligned}
        ...
        \Big(E_{x,i+\tfrac12,j,k+1}^{n}-E_{x,i+\tfrac12,j,k}^{n}\Big)                 \\
         & \quad
        +r_{z1}\psi^{n}_{h\,yx,i+\tfrac12,j,k+\tfrac12}
        -r_{z1}\psi^{n}_{h\,yz,i+\tfrac12,j,k+\tfrac12}.
    \end{aligned}
...
\begin{equation}
    \sigma_s=0, \qquad \alpha_s=0, \qquad \kappa_s=1, \qquad a_s=0.
\end{equation}
...
\begin{align}
    \begin{split}
        -\alpha_{x2}E_{x,i+\tfrac12,j,k-1}^{n+1} + \beta_{x2}E_{x,i+\tfrac12,j,k}^{n+1} - \gamma_{x2}E_{x,i+\tfrac12,j,k+1}^{n+1}
         ...
         & \quad -\frac{r_{x2}}{\kappa_{z,i+\tfrac12,j,k}\Delta z}\Big(\psi^{n+\frac12}_{h\,yx,i+\tfrac12,j,k+\tfrac12} - \psi^{n+\frac12}_{h\,yz,i+\tfrac12,j,k+\tfrac12}\Big)           \\
         & \quad +\frac{q_{x2}}{\kappa_{z,i+\tfrac12,j,k}\Delta z}\Big(\psi^{n+\frac12}_{h\,yx,i+\tfrac12,j,k-\tfrac12} - \psi^{n+\frac12}_{h\,yz,i+\tfrac12,j,k-\tfrac12}\Big)           \\
         & \quad +\Big(\psi^{n+\frac12}_{e\,xz,i+\tfrac12,j,k}-\psi^{n+\frac12}_{e\,xy,i+\tfrac12,j,k}\Big).
    \end{split}
...
\begin{equation}
    \begin{aligned}
        ...
        \Big(E_{x,i+\tfrac12,j,k+1}^{n+1}-E_{x,i+\tfrac12,j,k}^{n+1}\Big)             \\
         & \quad
        +r_{x2}\psi^{n+\frac12}_{h\,yx,i+\tfrac12,j,k+\tfrac12}
        -r_{x2}\psi^{n+\frac12}_{h\,yz,i+\tfrac12,j,k+\tfrac12}.
    \end{aligned}
...
\begin{align}
    \begin{split}
        -\alpha_{y2}E_{y,i-1,j+\tfrac12,k}^{n+1} + \beta_{y2}E_{y,i,j+\tfrac12,k}^{n+1} - \gamma_{y2}E_{y,i+1,j+\tfrac12,k}^{n+1}
         ...
         & \quad +\frac{q_{y2}}{\kappa_{x,i,j+\tfrac12,k}\kappa_{y,i-\tfrac12,j+\tfrac12,k}\Delta y\Delta x}\Big(E_{x,i-\tfrac12,j+1,k}^{n+\frac12}-E_{x,i-\tfrac12,j,k}^{n+\frac12}\Big) \\
         & \quad -\frac{r_{y2}}{\kappa_{x,i,j+\tfrac12,k}\kappa_{y,i+\tfrac12,j+\tfrac12,k}\Delta y\Delta x}\Big(E_{x,i+\tfrac12,j+1,k}^{n+\frac12}-E_{x,i+\tfrac12,j,k}^{n+\frac12}\Big) \\
         & \quad -\frac{r_{y2}}{\kappa_{x,i,j+\tfrac12,k}\Delta x}\Big(\psi^{n+\frac12}_{h\,zy,i+\tfrac12,j+\tfrac12,k} - \psi^{n+\frac12}_{h\,zx,i+\tfrac12,j+\tfrac12,k}\Big)           \\
         & \quad +\frac{q_{y2}}{\kappa_{x,i,j+\tfrac12,k}\Delta x}\Big(\psi^{n+\frac12}_{h\,zy,i-\tfrac12,j+\tfrac12,k} - \psi^{n+\frac12}_{h\,zx,i-\tfrac12,j+\tfrac12,k}\Big)           \\
...
\begin{equation}
    \begin{aligned}
        ...
        \Big(E_{y,i+1,j+\tfrac12,k}^{n+1}-E_{y,i,j+\tfrac12,k}^{n+1}\Big)             \\
         & \quad
        +r_{y2}\psi^{n+\frac12}_{h\,zy,i+\tfrac12,j+\tfrac12,k}
        -r_{y2}\psi^{n+\frac12}_{h\,zx,i+\tfrac12,j+\tfrac12,k}.
    \end{aligned}
...
\begin{align}
    \begin{split}
        -\alpha_{z2}E_{z,i,j-1,k+\tfrac12}^{n+1} + \beta_{z2}E_{z,i,j,k+\tfrac12}^{n+1} - \gamma_{z2}E_{z,i,j+1,k+\tfrac12}^{n+1}
         ...
         & \quad +\frac{q_{z2}}{\kappa_{y,i,j,k+\tfrac12}\kappa_{z,i,j-\tfrac12,k+\tfrac12}\Delta z\Delta y}\Big(E_{y,i,j-\tfrac12,k+1}^{n+\frac12}-E_{y,i,j-\tfrac12,k}^{n+\frac12}\Big) \\
         & \quad -\frac{r_{z2}}{\kappa_{y,i,j,k+\tfrac12}\kappa_{z,i,j+\tfrac12,k+\tfrac12}\Delta z\Delta y}\Big(E_{y,i,j+\tfrac12,k+1}^{n+\frac12}-E_{y,i,j+\tfrac12,k}^{n+\frac12}\Big) \\
         & \quad -\frac{r_{z2}}{\kappa_{y,i,j,k+\tfrac12}\Delta y}\Big(\psi^{n+\frac12}_{h\,xz,i,j+\tfrac12,k+\tfrac12} - \psi^{n+\frac12}_{h\,xy,i,j+\tfrac12,k+\tfrac12}\Big)           \\
         & \quad +\frac{q_{z2}}{\kappa_{y,i,j,k+\tfrac12}\Delta y}\Big(\psi^{n+\frac12}_{h\,xz,i,j-\tfrac12,k+\tfrac12} - \psi^{n+\frac12}_{h\,xy,i,j-\tfrac12,k+\tfrac12}\Big)           \\
...
\begin{equation}
    \begin{aligned}
        ...
        \Big(E_{z,i,j+1,k+\tfrac12}^{n+1}-E_{z,i,j,k+\tfrac12}^{n+1}\Big)             \\
         & \quad
        +r_{z2}\psi^{n+\frac12}_{h\,xz,i,j+\tfrac12,k+\tfrac12}
        -r_{z2}\psi^{n+\frac12}_{h\,xy,i,j+\tfrac12,k+\tfrac12}.
    \end{aligned}
...
\begin{equation}
    \frac{f_{\mathrm{ADI}}}{f_0}=\frac{2\arctan\!\left[S\sin(k\Delta z/2)\right]}{S\,k\Delta z}.
\end{equation}
...
\begin{equation}
    S_{E_y}(z,t) =E_0\frac{\Delta t}{T_P} \sin\!\left(\frac{2\pi z}{L_z}\right) \exp\!\left[-\frac{(t-3T_P)^2}{2T_P^2}\right] \sin(2\pi f_0t),
\end{equation}
...
\end{itemize}
In the macroscopic ADI push these two families are treated differently: soft electric sources are built into both implicit electric systems, while the entire magnetic source increment is applied after the first magnetic half-step. Hard electric sources are not supported in the ADI electric solve.

\subsection{Soft Electric Sources in the ADI Electric Update}

Write $S_{E_x,i+\tfrac12,j,k}^{n+\frac12}$ for the soft-source value assigned to the $E_x$ edge at $\bigl(i+\tfrac12,j,k\bigr)$ and the midpoint time level $n+\tfrac12$ (and likewise for $E_y$ and $E_z$ at their staggered locations). Over a full step the intended soft contribution at that edge is split equally between the two ADI half-steps, with both contributions evaluated at the common midpoint $n+\tfrac12$. The first-half electric update \eqref{eq:first-e-ex-adi} is then modified to
...
\begin{equation}
    \texttt{rhs}_{i+\tfrac12,j,k} \mathrel{+}= \tfrac12\,S_{E_x,i+\tfrac12,j,k}^{n+\frac12}/C_{b,i+\tfrac12,j,k}
\end{equation}
added after the curl terms have been assembled. The $E_y$ and $E_z$ first-half systems are modified in exactly the same way.

The second half-step is similar. For example, the $E_x$ system is
\begin{align}
    \begin{split}
        -\alpha_{x2}E_{x,i+\tfrac12,j,k-1}^{n+1}
        + \beta_{x2}E_{x,i+\tfrac12,j,k}^{n+1}
        - \gamma_{x2}E_{x,i+\tfrac12,j,k+1}^{n+1}
         & = p_{x2}E_{x,i+\tfrac12,j,k}^{n+\frac12}                                                                               \\
         & \quad +\frac{1}{\Delta y}\Big(H_{z,i+\tfrac12,j+\tfrac12,k}^{n+\frac12}-H_{z,i+\tfrac12,j-\tfrac12,k}^{n+\frac12}\Big) \\
         & \quad -\frac{1}{\Delta z}\Big(H_{y,i+\tfrac12,j,k+\tfrac12}^{n+\frac12}-H_{y,i+\tfrac12,j,k-\tfrac12}^{n+\frac12}\Big) \\
         & \quad +\frac{q_{x2}}{\Delta x\Delta z}\Big(E_{z,i+1,j,k-\tfrac12}^{n+\frac12}-E_{z,i,j,k-\tfrac12}^{n+\frac12}\Big)    \\
         ...
         & \quad +\frac{1}{2C_{b,i+\tfrac12,j,k}}\,S_{E_x,i+\tfrac12,j,k}^{n+\frac12}.
    \end{split}
    \label{eq:second-e-ex-adi-tridiagonal-excitation}
\end{align}
Thus, after the second-half curl terms have been assembled, the normalized right-hand side is changed by
\begin{equation}
    \texttt{rhs}_{i+\tfrac12,j,k} \mathrel{+}= \tfrac12\,S_{E_x,i+\tfrac12,j,k}^{n+\frac12}/C_{b,i+\tfrac12,j,k}.
\end{equation}
The $E_y$ and $E_z$ second-half systems are modified in exactly the same way.
...
\subsection{Magnetic Sources After the First Half-Step}

After the first ADI half-step has produced $\mathbf{E}^{n+\frac12}$ and $\mathbf{B}^{n+\frac12}$, an external magnetic excitation is applied directly to the magnetic field using the source sample evaluated at the midpoint $n+\tfrac12$. The entire full-step source increment is applied at this point so that it enters the second electric solve. Thus, a soft magnetic source (flag $=2$) contributes
\begin{equation}
...
\begin{equation}
    ...
    S_{B_x,i,j+\tfrac12,k+\tfrac12}^{n+\frac12}.
\end{equation}
No additional magnetic excitation is applied after the second half-step. The same rules apply to $B_y$ and $B_z$ at their staggered faces. Unlike the electric soft source, this magnetic update is an explicit modification of the magnetic field rather than a term in a tridiagonal electric system; applying it after the first magnetic half-step allows it to affect the second electric solve.
...
\begin{enumerate}
    ...
    \item Apply the entire magnetic excitation to $\mathbf{B}^{n+\frac12}$ (soft: add $S_B^{n+\frac12}$; hard: set equal to $S_B^{n+\frac12}$).
    \item Form and solve the second-half electric systems with the same midpoint soft electric sources added as $\tfrac12 S_E^{n+\frac12}$.
    \item Update $\mathbf{B}$ to $n+1$ without applying another magnetic excitation.
\end{enumerate}
