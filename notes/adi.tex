...
\end{equation}
Thus $\kappa$ modifies the local diagonal coefficients $C_a^{L}$ and $C_b^{L}$, while the known current $J_{\mathrm{L}}^{n}$ enters the right-hand side. After $E_x^{n+\frac12}$ is obtained, rearrange \eqref{eq:inductor-trapezoidali} to advance $J_{\mathrm{L},x}$ by
\begin{equation}
