...
\end{equation}
added after the curl terms have been assembled. The $E_y$ and $E_z$ first-half systems are modified in exactly the same way, and the second half-step uses the same midpoint sample, with $S_{E_x,i+\tfrac12,j,k}^{n+\frac12}/(2C_{b,i+\tfrac12,j,k})$ on the normalized right-hand side.
