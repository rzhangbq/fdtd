...
After the first ADI half-step has produced $\mathbf{E}^{n+\frac12}$ and $\mathbf{B}^{n+\frac12}$, and again after the second half-step has produced $\mathbf{E}^{n+1}$ and $\mathbf{B}^{n+1}$, an external magnetic excitation is applied directly to the magnetic field. Both contributions use the same source sample evaluated at the common midpoint $n+\tfrac12$. Thus, after the first half-step, a soft magnetic source (flag $=2$) contributes
\begin{equation}
...
\begin{enumerate}
    ...
    \item Form and solve the second-half electric systems using the same midpoint soft electric sources $S_E^{n+\frac12}$.
    \item Update $\mathbf{B}$ to $n+1$, then apply magnetic excitation evaluated at the same midpoint $n+\tfrac12$.
\end{enumerate}
